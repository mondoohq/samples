...
$mondooConfigPath = 'C:\Windows\System32\config\systemprofile\.config\mondoo\mondoo.yml'

try {
    Set-ExecutionPolicy Bypass -Scope Process -Force
    [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072

    # Download and load the Mondoo install script (defines Install-Mondoo function)
    ...
    $env:PATH = "C:\Program Files\Mondoo;$env:PATH"

    # Register with Mondoo
    if ($mondooToken -ne '') {
        & $cnspecPath login --token $mondooToken
        Write-Host 'cnspec registered with Mondoo'
        Write-Host "Mondoo config written to: $mondooConfigPath"

        # Run initial scan with explicit config path
        Write-Host '=== Running initial cnspec scan ==='
        & $cnspecPath scan local --config $mondooConfigPath
        Write-Host 'Initial cnspec scan complete'

        # Register Mondoo as a Windows service
        Write-Host '=== Registering Mondoo service ==='
        & $cnspecPath service install --config $mondooConfigPath
        Set-Service -Name mondoo -Status Running -StartupType Automatic
        Write-Host 'Mondoo service registered and set to auto-start'
    } else {
        Write-Host '[SKIP] No Mondoo token provided - skipping registration'
    }
} catch {
